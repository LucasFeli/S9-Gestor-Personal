        ...
        <ul>
          {expenses.map(expense => (
            <li key={expense.id} className="mb-4 p-4 border rounded-md">
              <h3 className="text-xl font-bold">Ingreso Total Mensual :{" "} {expense.gastos.find(g => g.descripcion === 'Ingresos Totales').cantidad} {expense.moneda}</h3>
              ...
              <div className="mt-4">
              <Link to={`/expense/${expense.id}`} className=" bg-white text-blue-500  px-4 py-2 rounded hover:bg-yellow-500">Ver Detalles</Link>
              <button
              ...
              <Link to={`/edit-expense/${expense.id}`} className="ml-4 bg-green-500 text-white px-4 py-2 rounded hover:bg-green-600">
